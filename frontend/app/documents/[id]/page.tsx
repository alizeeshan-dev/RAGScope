  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!target && requestedChunk) {
        const chunk = await api.getChunk(requestedChunk);
      ...
      const resolved = target ?? els[0] ?? null;
      setDocument(doc); setElements(els); setArtifacts(storedArtifacts); setSelected(resolved);
      if (resolved) window.setTimeout(() => window.document.getElementById(`element-${resolved.id}`)?.scrollIntoView({ block: "center" }), 0);
